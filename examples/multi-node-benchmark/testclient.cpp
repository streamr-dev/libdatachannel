...
void TestClient::start(const string& id, const string& wsUrl)
    {
    ...
    ws->onMessage([connections = &this->connections, id, ws=&this->ws ] (variant<binary, string> data)
        {
        ...
        if (type =="connect")
            ...
            shared_ptr<Peer> peer = make_shared<Peer>(id, target, *ws);
